...
Save like PayPal. Settle on Arbitrum.

VaultPay is a simple crypto savings app built on Arbitrum. It lets users create named savings vaults, add funds over time, and withdraw based on transparent maturity and penalty rules enforced by a smart contract.

Unlike complex DeFi dashboards, VaultPay is designed to feel familiar. The interface focuses on the actions users actually need: connect wallet, join, create a saving, add funds, view maturity, and withdraw.

## Builderthon Focus

VaultPay demonstrates how onchain financial applications can feel as simple as consumer fintech products while still benefiting from transparent smart contract execution on Arbitrum Stylus and Rust.

## Frontend Demo

The dependency-free frontend lives in `frontend/`.
...
Configure the deployed contract and Arbitrum network details in `frontend/config.js`.
