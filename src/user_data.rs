...
impl UserData {
    ...
    fn calculate_penalty(amount: U256, penalty_perc: U8) -> U256 {
        amount * U256::from(penalty_perc) / U256::from(100)
    }
    ...
    pub fn create_saving_data(
        ...
        let start_time = self.vm().block_timestamp();
        ...
        new_saving.start_time.set(U256::from(start_time));
    ...
    pub fn increment_saving_data(
        ...
        if saving_data.token_id.get() != token_id {
    ...
    pub fn withdraw_saving_data(&mut self, name_of_saving: String) -> Result<(U256, U256), Vec<u8>> {
        let saving_data = self.savings_map.get(name_of_saving.clone());
        if !saving_data.is_valid.get() {
            return Err(format!("Saving `{}` doesn't exist", name_of_saving).into());
        }
        ...
        let penalty_amount = if saving_data.maturity_time.get()
            > U256::from(self.vm().block_timestamp())
        {
            // Saving is not mature yet, so the early withdrawal penalty applies.
            Self::calculate_penalty(saving_amount, saving_data.penalty_perc.get())
        } else {
            U256::from(0)
        };
        let withdraw_amount = saving_amount - penalty_amount;
        ...
        Ok((withdraw_amount, penalty_amount))
    }
}
